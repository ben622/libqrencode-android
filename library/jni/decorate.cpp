...
void qrencode::decorate::insertLogo(JNIEnv *env, AndroidBitmapInfo *pixelInfo, void *pixels,
                                    jobject jlogo) {
    if (jlogo == NULL) {
        if (LOGS_ENABLED) LOGE("%s", "insert logo is null");
        return;
    }
    ...
    if (result != ANDROID_BITMAP_RESULT_SUCCESS) {
        if (LOGS_ENABLED) LOGE("%s", "insert logo::AndroidBitmap_getInfo failed");
        return;
    }
    ...
    if (result != ANDROID_BITMAP_RESULT_SUCCESS) {
        if (LOGS_ENABLED) LOGE("%s", "insert logo::AndroidBitmap_lockPixels failed");
        return;
    }
    ...
    for (int y = sy; y < sy + logoBitmapInfo.height; y++) {
        for (int x = sx; x < sx + logoBitmapInfo.width; x++) {
            ...
            draw(pixelInfo->stride, pixels, x, y,
